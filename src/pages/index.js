  ...
  useEffect(() => {
    const fetchLogos = async () => {
      try {
        ...
        if (data.success) {
          const analytical = data.data.filter((logo) => logo.name === "analytical");
  ...
  return (
    <Container sx={{ paddingY: 4, minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <Box sx={{ textAlign: "center", marginBottom: 4 }}>
        <Image src={companyLogo} alt="Company Logo" width={150} height={150} />
      </Box>
        ...
        <Table>
          ...
          <TableBody>
            {logos.analytical.length > 0 || logos.process.length > 0 ? (
              ...
              <TableRow>
                <TableCell colSpan={2} align="center" sx={{ py: 3, fontWeight: "bold", fontSize: "16px" }}>
                  No logos available.
                </TableCell>
              </TableRow>
            )}
      ...
      {/* Modal for Iframe */}
      <ModalComponent open={open} handleClose={handleClose} iframeSrc={iframeSrc} />
    </Container>
  );
};
